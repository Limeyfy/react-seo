...
const CheckTitles = () => {
  ...
  if (titles?.length > 1) {
    consoleWarn(
      `Too many <h1> tags, you have ${titles.length}. It should be 1`
    );
    return false;
  }
...
const CheckPageTitle = () => {
  ...
  if (!pageTitle) {
    consoleWarn(
      "Page should have a <title> tag. Use title value in DefaultHelmet"
    );
    return false;
  }
  if (pageTitle.length <= 7 || pageTitle.length >= 70) {
    consoleWarn(
      `You have a title tag of optimal length (between 10 and 70 characters). Current description length: ${pageTitle.length}`
    );
    return false;
  }
...
const CheckDescription = () => {
  ...
  if (!description) {
    consoleWarn(
      "Page should have a <meta> tag with description. Use description value in DefaultHelmet"
    );
    return false;
  }
  ...
  if (descriptionText.length < 71 || descriptionText.length > 320) {
    consoleWarn(
      `Your page has a meta description of optimal length (between 70 and 320 characters). Current description length: ${descriptionText.length}`
    );
    return false;
  }
...
const CheckKeywords = () => {
  ...
  if (!keywords) {
    consoleWarn(
      "Page should have a <meta> tag with keywords. Use keywords value in DefaultHelmet"
    );
    return false;
  }
  const newLocal: any = "content";
...
const CheckImages = () => {
  ...
  if (imagesWithNoAlt.length > 0) {
    ...
    consoleWarn(
      "You have images with no alt attribute. Images listed in console log above."
    );
    return false;
  }
...
export const CheckSeo = () => {
  ...
  if (CheckKeywords()) Works.push("Keywords");
  ...
  consoleInfo(
    `Report finished. ${word && word[Math.floor(Math.random() * word.length)]}`
  );
};
